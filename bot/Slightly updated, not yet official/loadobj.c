...
//	theMesh->vertexToTriangleTable = NULL;
	...
	// Allocate arrays!
	if (vertCount > 0)
		theMesh->vertices = (float *)malloc(sizeof(GLfloat) * vertCount);
	if (texCount > 0)
		theMesh->textureCoords = (float *)malloc(sizeof(GLfloat) * texCount);
	if (normalsCount > 0)
		theMesh->vertexNormals = (float *)malloc(sizeof(GLfloat) * normalsCount);
...
Mesh **SplitToMeshes(Mesh *m)
{
	...
	for (i = 0; i < m->groupCount+1; i++)
	{
		...
		mm[i]->vertices = (float *)malloc(m->vertexCount * sizeof(GLfloat) * 3);
		if (m->normalsCount > 0)
			mm[i]->vertexNormals = (float *)malloc(m->normalsCount * sizeof(GLfloat) * 3);
		else
			mm[i]->vertexNormals = NULL;
		if (m->texCount > 0)
			mm[i]->textureCoords = (float *)malloc(m->texCount * sizeof(GLfloat) * 2);
		else
